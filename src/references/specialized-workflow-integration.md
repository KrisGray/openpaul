...
SPECIAL-FLOWS enables explicit tracking of specialized skills, commands, and workflows within OpenPAUL governance. Instead of ad-hoc skill invocation, projects declare which skills apply, when they're required, and UNIFY verifies they were actually used.
...
### The Solution
Layered reinforcement through the OpenPAUL loop:
...
### With Init
During `npx openpaul` initialization, optionally configure specialized flows:
- "Do you have specialized skills for this project?"
- If yes, route to configuration workflow
- If no, skip (can add later via `/openpaul:flows`)
