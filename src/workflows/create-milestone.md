...
@.openpaul/STATE.md
@.openpaul/ROADMAP.md
@.openpaul/MILESTONE-CONTEXT.md (if exists)
</required_reading>
...
1. Check for MILESTONE-CONTEXT.md:
   ```bash
   cat .openpaul/MILESTONE-CONTEXT.md 2>/dev/null
   ```
...
3. **Add phase details section:**
   ...
   Focus: {description}
    Plans: TBD (defined during /openpaul:plan)
   ```
...
```bash
mkdir -p .openpaul/phases/{NN}-{name-slug}
```
...
Next action: /openpaul:plan for Phase {first_phase_number}
...
```bash
rm .openpaul/MILESTONE-CONTEXT.md
```
...
Created:
  .openpaul/phases/{phase-1-slug}/     ✓
  .openpaul/phases/{phase-2-slug}/     ✓
  .openpaul/phases/{phase-N-slug}/     ✓
...
- Phase directories created in .openpaul/phases/
