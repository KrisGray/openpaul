<purpose>
Configure specialized skill integrations for an OpenPAUL project. Discovers available skills, prompts user to select applicable ones, maps skills to work types, and generates .openpaul/SPECIAL-FLOWS.md.
</purpose>

<when_to_use>
- During init when user has specialized skills/commands
- Ad-hoc via `/openpaul:flows` command
...
<required_reading>
@.openpaul/PROJECT.md (project context)
@.openpaul/SPECIAL-FLOWS.md (if exists, for amendment)
</required_reading>
...
1. Check if .openpaul/SPECIAL-FLOWS.md exists:
   ```bash
   ls .openpaul/SPECIAL-FLOWS.md 2>/dev/null
   ```
2. If exists:
   ```
...
1. Create .openpaul/SPECIAL-FLOWS.md using template:
...
Created: .openpaul/SPECIAL-FLOWS.md
...
Run /openpaul:flows audit to check current phase.
   ────────────────────────────────────────
   ```
</step>
...
1. Read .openpaul/SPECIAL-FLOWS.md
2. Read .openpaul/STATE.md for current phase
...
<output>
- `.openpaul/SPECIAL-FLOWS.md` created or updated
- `.openpaul/PROJECT.md` updated with quick reference
