...
```bash
ls .openpaul/debug/*.md 2>/dev/null | grep -v resolved
```
...
Write to `.openpaul/debug/[slug].md`
...
```bash
mkdir -p .openpaul/debug/resolved
mv .openpaul/debug/[slug].md .openpaul/debug/resolved/
```
...
Debug session: .openpaul/debug/resolved/[slug].md"
```
...
Session archived: .openpaul/debug/resolved/[slug].md
```
</step>
