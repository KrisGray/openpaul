...
### Source Reference
- **GSD File:** `get-shit-done/workflows/debug.md`

### Adapted from GSD
- Philosophy (user = reporter, Claude = investigator)
- Process steps (check_active_session through archive_session)
- Symptom gathering via AskUserQuestion
- Investigation loop with hypothesis testing
- Resume behavior from debug file
- Update rules (OVERWRITE/APPEND/IMMUTABLE)
- Context management guidance

### PAUL Innovations
- **Directory:** `.paul/debug/` instead of `.planning/debug/`
- **Loop integration:** Complex fixes can trigger dedicated PLAN.md
- **Session language:** "context reset" instead of "/clear"
- **Template reference:** `@src/templates/DEBUG.md` path
