...
```bash
find .openpaul/phases -name "*SUMMARY.md" -type f -exec ls -lt {} + | head -5
```
...
1. Create `.openpaul/phases/XX-name/{phase}-{plan}-UAT.md` if doesn't exist
...
**Note:** Issues go to phase-scoped UAT file, NOT global `.openpaul/ISSUES.md`.
</step>
