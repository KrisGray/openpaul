...
Create `.openpaul/phases/{NN}-{name}/CONTEXT.md`:

Use CONTEXT.md template format.

Display:
```
Context saved to .openpaul/phases/{NN}-{name}/CONTEXT.md
...
**Accept:** "yes", "go", "plan" → run `/openpaul:plan`
</step>

</process>

<output>
- .openpaul/phases/{NN}-{name}/CONTEXT.md created (handoff file)
- Goals and approach articulated
- Ready for /openpaul:plan to consume
</output>
...
- [ ] Clear handoff to /openpaul:plan
</success_criteria>
...
- Route directly to /openpaul:plan
