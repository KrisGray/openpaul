...
1. Check for global issues:
   ```bash
   ls .openpaul/ISSUES.md 2>/dev/null
   ```

2. Check for phase-scoped UAT files:
   ```bash
   find .openpaul/phases -name "*-UAT.md" -type f 2>/dev/null
   ```
...
3. If no issue files found:
   ...
   - Deferred enhancements during development → .openpaul/ISSUES.md
   - UAT testing findings → .openpaul/phases/XX-name/{plan}-UAT.md
...
- Suggest command: `/openpaul:plan 10.3 Address urgent issues [list]`
