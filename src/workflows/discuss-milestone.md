...
- Before /openpaul:milestone when planning is desired
</when_to_use>

<loop_context>
N/A - This is a pre-planning workflow, not a loop phase.
After discussion, routes to /openpaul:milestone (create-milestone).
</loop_context>

<required_reading>
@.openpaul/STATE.md
@.openpaul/PROJECT.md (current requirements and progress)
@.openpaul/MILESTONES.md (previous milestone accomplishments, if exists)
</required_reading>
...
Create `.openpaul/MILESTONE-CONTEXT.md`:

```markdown
# Milestone Context

**Generated:** {date}
**Status:** Ready for /openpaul:milestone
...
*This file is temporary. It will be deleted after /openpaul:milestone creates the milestone.*
```

Display:
```
Context saved to .openpaul/MILESTONE-CONTEXT.md
...
**Accept:** "yes", "go", "create" → run `/openpaul:milestone`
</step>

</process>

<output>
- .openpaul/MILESTONE-CONTEXT.md created (handoff file)
- Features, scope, and phases articulated
- Ready for /openpaul:milestone to consume
</output>
...
- [ ] Clear handoff to /openpaul:milestone
</success_criteria>
...
- Route directly to /openpaul:milestone
