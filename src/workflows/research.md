...
<required_reading>
@openpaul/src/references/subagent-criteria.md
</required_reading>
...
Examples:
  ...
  /openpaul:research "auth patterns in this codebase" --codebase
  /openpaul:research "prisma vs drizzle comparison" --web
```
Exit workflow.
...
Create `.openpaul/research/{topic-slug}.md`:

1. Create research directory if needed:
   ```bash
   mkdir -p .openpaul/research
   ```
...
Topic: {topic}
Agent: {agent_type}
Output: .openpaul/research/{topic-slug}.md
...
<output>
- .openpaul/research/{topic-slug}.md created with findings
