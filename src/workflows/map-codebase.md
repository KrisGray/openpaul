...
### Source Reference
- **GSD File:** `~/.claude/get-shit-done/workflows/map-codebase.md`

### Adapted from GSD
- Overall workflow structure (check_existing, create_structure, spawn_agents, collect_results, write_documents, verify_output)
- 4-agent parallel analysis pattern with domain focus
- Agent prompts emphasizing file paths
- Template filling process
- Completion summary format

### PAUL Innovations
- **Output directory:** `.paul/codebase/` instead of `.planning/codebase/`
- **STATE.md awareness:** Added update_state step to note mapping in STATE.md
- **PAUL command references:** Points to `/paul:init` instead of `/gsd:new-project`
- **Template references:** Uses `src/templates/codebase/` path structure
- **Commit message:** Includes Claude co-author attribution per PAUL conventions
