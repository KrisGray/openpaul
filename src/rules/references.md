...
### Source Reference
- **GSD File:** `~/.claude/get-shit-done/.claude/rules/references.md`

### Adapted from GSD
- No frontmatter rule
- Outer container pattern (semantic XML matching filename)
- Teaching patterns (contrast, explain WHY)
- Key principle: References explain concepts loaded when relevant

### PAUL Adaptations
- **File structure:** Defined explicit section order (outer container, purpose, concepts, examples, anti-patterns)
- **Loading pattern:** Documented how workflows @-reference references with conditions
- **Self-contained:** Added requirement that references should work standalone
- **Negative definition:** Added "References are NOT" list to clarify boundaries with workflows, templates, CARL
- **Path pattern:** `src/references/` instead of `get-shit-done/references/`
