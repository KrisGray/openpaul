...
### Source Reference
- **GSD File:** `~/.claude/get-shit-done/templates/state.md`

### Adapted from GSD
- Project Reference section (pointer to PROJECT.md with core value)
- Current Position section (phase/plan/status/progress)
- Performance Metrics section (velocity tracking, per-phase breakdown)
- Accumulated Context structure (decisions, deferred issues, blockers)
- Session Continuity section (resume information)
- Size constraint (<100 lines)
- Lifecycle patterns (creation, reading, writing triggers)

### PAUL Innovations Beyond GSD
- **Loop Position indicator** - Visual representation of PLAN/APPLY/UNIFY state. GSD doesn't have explicit loop visualization.
- **Boundaries (Active) section** - Quick reference to current phase protections. GSD references this only in plans.
- **Status values aligned to PAUL phases** - "Planning | Approved | Applying | Unifying" vs GSD's more generic states.

### Sources for PAUL Additions
- Loop visualization: Original PAUL concept for explicit phase awareness
- Active boundaries: Derived from "DO NOT CHANGE" pattern elevated to state tracking
