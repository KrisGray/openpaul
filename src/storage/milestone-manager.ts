...
export class MilestoneManager {
   ...
   */
  private getRoadmapPath(): string | null {
   ...
   */
  private formatPhaseRange(phases: number[]): string {
   ...
   */
  private formatMilestoneDetailSection(milestone: Milestone, existingPhases: PhaseEntry[]): string {
...
`
   ...
   */
  private formatScopeItems(phases: number[], existingPhases: PhaseEntry[]): string {
    return phases
      .sort((a, b) => a - b)
      .map(phaseNum => {
        const phase = existingPhases.find(p => p.number === phaseNum)
  ...
  getMilestone(name: string): Milestone | null {
    ...
    const milestoneRegex = new RegExp(
      `###\\s*[✅🚧📋]+\\s*${this.escapeRegex(name)}\\s*\\((planned|in[- ]progress|completed)\\)`,
      'i'
    )
   ...
   */
  private parseMilestoneFromContent(content: string, name: string): Milestone | null {
    // Find the milestone section
    const sectionRegex = new RegExp(
      `###\\s*[✅🚧📋]+\\s*${this.escapeRegex(name)}\\s*\\([^)]*\\)[\\s\\S]*?(?=###\\s*[✅🚧📋]|##\\s*Phases|##\\s*Progress|##\\s*Dependencies|$)`,
      'i'
    )
    ...
    const status = statusMatch 
      ? (statusMatch[1].toLowerCase().replace(' ', '-') as 'planned' | 'in-progress' | 'completed')
      : 'planned'
    ...
    const progressMatch = content.match(new RegExp(
      `\\*\\*Milestone:\\*\\*\\s*${this.escapeRegex(name)}[\\s\\S]*?\\*\\*Started:\\*\\*\\s*(.+?)(?:\n|$)`,
      'i'
    ))
  ...
  getMilestoneProgress(name: string): MilestoneProgress | null {
    ...
    const phaseStatus: PhaseStatus[] = milestone.phases.map(phaseNum => {
  ...
  completeMilestone(name: string): MilestoneArchiveEntry {
    ...
    const archiveEntry: MilestoneArchiveEntry = {
      name: milestone.name,
   ...
   */
  private calculatePlansCompleted(phases: PhaseEntry[]): number {
    // Count completed plans from phase sections
    let completed = 0
    const roadmapPath = this.getRoadmapPath()
    if (!roadmapPath) return 0
    ...
    for (const phase of phases) {
      if (phase.status === 'complete') {
        ...
        const phaseSectionRegex = new RegExp(
          `###\\s*Phase\\s+${phase.number}:[\\s\\S]*?(?=###\\s*Phase|$)`,
          'i'
        )
   ...
   */
  private calculateTotalPlans(phases: PhaseEntry[]): number {
    let total = 0
    const roadmapPath = this.getRoadmapPath()
    if (!roadmapPath) return 0
   ...
   */
  private calculateExecutionTime(startedAt: string | null, completedAt: string): string {
    if (!startedAt) {
      return 'Unknown'
    }
   ...
   */
  private extractRequirements(phases: PhaseEntry[]): string[] {
    const requirements: string[] = []
    const roadmapPath = this.getRoadmapPath()
    if (!roadmapPath) return requirements
   ...
   */
  private archiveMilestone(entry: MilestoneArchiveEntry, phases: PhaseEntry[]): void {
...
`
  ...
  private collapseMilestoneInRoadmap(
    roadmapPath: string, 
    milestone: Milestone, 
    archiveEntry: MilestoneArchiveEntry,
    phases: PhaseEntry[]
  ): void {
    let content = readFileSync(roadmapPath, 'utf-8')

    // Update milestone status in Milestones section
    const milestoneLineRegex = new RegExp(
      `(-\\s*)[🚧📋](\\s*\\*\\*${this.escapeRegex(milestone.name)}\\*\\*\\s*-\\s*Phases\\s+[^\\(]+\\()([a-z-]+)(\\))`,
      'i'
    )
    ...
    const detailSectionRegex = new RegExp(
      `(###\\s*[✅🚧📋]+\\s*${this.escapeRegex(milestone.name)}\\s*\\([^)]*\\)[\\s\\S]*?(?=###\\s*[✅🚧📋]|##\\s*Phases|##\\s*Progress|##\\s*Dependencies|$))`,
      'i'
    )
...
`
   ...
   */
  private getPlanCountForPhase(phaseNumber: number): number {
    const roadmapPath = this.getRoadmapPath()
    if (!roadmapPath) return 0

    const content = readFileSync(roadmapPath, 'utf-8')
    const phaseSectionRegex = new RegExp(
      `###\\s*Phase\\s+${phaseNumber}:[\\s\\S]*?(?=###\\s*Phase|$)`,
      'i'
    )
    const sectionMatch = content.match(phaseSectionRegex)
    if (sectionMatch) {
