...
**When to use:** After `/openpaul:verify` logs issues to phase-scoped UAT file.
...
@~/.claude/openpaul-framework/references/checkpoints.md
</execution_context>
...
Usage: /openpaul:plan-fix 04-02

This creates a fix plan from .openpaul/phases/XX-name/{plan}-UAT.md
```
Exit.
</step>
...
```bash
ls .openpaul/phases/*/{plan}-UAT.md 2>/dev/null
```
...
UAT.md files are created by /openpaul:verify when testing finds issues.
...
Create `.openpaul/phases/XX-name/{plan}-FIX.md`:
...
**Issues being fixed:**
@.openpaul/phases/XX-name/{plan}-UAT.md

**Original plan for reference:**
@.openpaul/phases/XX-name/{plan}-PLAN.md
</context>
...
- Ready for re-verification with /openpaul:verify
</success_criteria>

<output>
After completion, create `.openpaul/phases/XX-name/{plan}-FIX-SUMMARY.md`
</output>
```
</step>
...
If approved: `/openpaul:apply .openpaul/phases/XX-name/{plan}-FIX.md`
</step>
...
- [ ] FIX.md written with proper OpenPAUL structure
