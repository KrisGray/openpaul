...
### Source Reference
- **GSD File:** `commands/gsd/plan-fix.md`

### Adapted from GSD
- Argument parsing pattern (plan number required)
- Issue file discovery via glob
- Issue parsing (ID, title, severity, description)
- Task generation from issues
- Priority ordering (blocker → major → minor)
- FIX.md structure with frontmatter
- Execution offer at end

### PAUL Adaptations
- **Directory structure:** Uses `.paul/phases/` instead of `.planning/phases/`
- **File naming:** Uses `{plan}-UAT.md` instead of `{plan}-ISSUES.md`
- **Severity labels:** Uses PAUL conventions (Blocker/Major/Minor/Cosmetic)
- **FIX.md structure:** Includes full PAUL template (acceptance_criteria, boundaries sections)
- **Quick continuation:** Matches PAUL [1]/[2]/[3] prompt pattern
