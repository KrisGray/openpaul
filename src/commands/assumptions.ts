...
import type { AssumptionEntry } from '../types/pre-planning'
import type { ValidationStatus } from '../types/pre-planning'
import type { ConfidenceLevel } from '../types/pre-planning'

type AssumptionsArgs = {
  phase: number
  assumptions: string
  statuses?: string
  confidences?: string
...
export const openpaulAssumptions = toolFactory({
  ...
  execute: async (args: AssumptionsArgs, context: { directory: string }) => {
    try {
      ...
      const entries: AssumptionEntry[] = assumptionStatements.map((statement, i) => {
        const status = statusValues[i] || 'unvalidated'
        const confidence = confidenceValues[i] || 'medium'
        return {
          statement,
          validation_status: validStatuses.includes(status) ? (status as ValidationStatus) : 'unvalidated',
          confidence: validConfidences.includes(confidence) ? (confidence as ConfidenceLevel) : 'medium',
