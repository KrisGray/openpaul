...
### Source Reference
- **GSD Command:** `/gsd:map-codebase` (implicit in workflow usage)

### Adapted from GSD
- Thin command wrapper pattern
- Delegation to workflow for execution
- Success criteria format

### PAUL Innovations
- **Command path:** `src/commands/map-codebase.md` vs GSD's commands/gsd/ structure
- **Output directory:** `.paul/codebase/` referenced in objective
- **Explicit agent breakdown:** Lists the 4 agent types in process section
- **Workflow reference:** Uses `@src/workflows/` path convention
