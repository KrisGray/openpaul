...
export class QualityManager {
    ...
    resolvePhaseDir(phaseNumber) {
        ...
        const matchingDir = phaseDirs.find(dir => dir.startsWith(`${phaseNumber.toString().padStart(2, '0')}-`));
        if (matchingDir) {
     ...
     */
    parseSummaryMustHaves(phaseDir) {
        if (!existsSync(phaseDir)) {
            return null;
        }
        const files = readdirSync(phaseDir);
     ...
     */
    async writeUAT(phaseDir, uat) {
        const validated = UATSchema.parse(uat);
        const uatPath = join(phaseDir, 'UAT.md');
    ...
    readUATIssues(phaseDir) {
        ...
        try {
            ...
            const data = JSON.parse(jsonMatch[1]);
            return UATIssuesSchema.parse(data);
        }
        catch (error) {
     ...
     */
    async writeUATIssues(phaseDir, issues) {
        const validated = UATIssuesSchema.parse(issues);
        const issuesPath = join(phaseDir, 'UAT-ISSUES.md');
        // Generate markdown content
     ...
     */
    generateUATIssuesMarkdown(issues) {
        const lines = [
            '# UAT Issues',
            '',
     ...
     */
    getNextAlphaPlanId(phaseDir, basePlanId) {
        if (!existsSync(phaseDir)) {
            return `${basePlanId}a`;
        }
        const files = readdirSync(phaseDir);
     ...
     */
    createEmptyUAT(phaseNumber, planId) {
        return {
            phaseNumber,
            planId,
            testedAt: Date.now(),
     ...
     */
    createEmptyUATIssues(phaseNumber, sourcePlanId) {
        return {
            phaseNumber,
            sourcePlanId,
            createdAt: Date.now(),
