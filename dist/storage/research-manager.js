...
export class ResearchManager {
    ...
    resolveResearchPath(phaseNumber) {
    ...
    createResearchResult(phaseNumber, query, findings) {
    ...
    calculateOverallConfidence(findings) {
        ...
        const totalScore = findings.reduce((sum, f) => sum + confidenceScores[f.confidence], 0);
    ...
    aggregateAgentResults(agentResults) {
        ...
        const findings = completedAgents
            .filter((a) => a.summary)
            .map((a) => ({
            topic: a.topic,
            summary: a.summary,
    ...
    formatAgentDashboard(dashboard) {
        const lines = ['## Research Progress', ''];
    ...
    generateResearchContent(result) {
        const lines = [
    ...
    generateResearchPhaseContent(phaseNumber, result, findings, themes) {
        const lines = [
        ...
        for (const finding of result.findings) {
            lines.push(`| ${finding.topic} | ✅ Complete | ${finding.confidence.toUpperCase()} | ${finding.summary.slice(0, 50)} |`);
        }
        lines.push('');
        if (result.themes.length > 0) {
            ...
            for (const theme of result.themes) {
                ...
                if (themeFindings.length > 0) {
                    ...
                    lines.push('');
                    lines.push(`**From:** ${themeFindings.map((f) => f.topic).join(', ')}`);
                    lines.push('');
    ...
    async writeResearch(phaseNumber, query, findings) {
        const phaseDir = this.resolvePhaseDir(phaseNumber);
        if (!phaseDir) {
    ...
    async writeResearchPhaseResult(phaseNumber, result, findings, themes) {
        const phaseDir = this.resolvePhaseDir(phaseNumber);
        if (!phaseDir) {
            throw new Error(`Phase ${phaseNumber} directory not found`);
        }
