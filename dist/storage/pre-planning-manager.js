...
export class PrePlanningManager {
    ...
    formatPhaseDirName(phaseNumber) {
        return `${phaseNumber.toString().padStart(2, '0')}-phase-${phaseNumber}`;
    }
    resolveContextPath(phaseNumber) {
    ...
    resolveAssumptionsPath(phaseNumber) {
        const phaseDir = this.resolvePhaseDir(phaseNumber);
        if (!phaseDir)
            return null;
    ...
    resolveDiscoveryPath(phaseNumber) {
        const phaseDir = this.resolvePhaseDir(phaseNumber);
        if (!phaseDir)
            return null;
    ...
    createContext(phaseNumber, params) {
    ...
    generateContextContent(artifact) {
        const lines = [
    ...
    createAssumptions(phaseNumber, assumptions) {
        const now = new Date().toISOString();
        const artifact = {
            phase: phaseNumber,
            assumptions,
    ...
    generateAssumptionsContent(artifact) {
        const lines = [
        ...
        for (const assumption of artifact.assumptions) {
            lines.push(`| ${assumption.statement} | ${assumption.validation_status} | ${assumption.confidence} | ${assumption.impact} |`);
        }
        lines.push('');
    ...
    createIssues(phaseNumber, issues) {
        const now = new Date().toISOString();
        const artifact = {
            phase: phaseNumber,
    ...
    generateIssuesContent(artifact) {
        const lines = [
    ...
    createDiscovery(phaseNumber, params) {
        const now = new Date().toISOString();
        const artifact = {
            phase: phaseNumber,
            depth: params.depth,
    ...
    generateDiscoveryContent(artifact, topic) {
        const lines = [
    ...
    async writeContext(phaseNumber, params) {
        const phaseDir = this.resolvePhaseDir(phaseNumber);
        if (!phaseDir) {
    ...
    async writeAssumptions(phaseNumber, assumptions) {
        const phaseDir = this.resolvePhaseDir(phaseNumber);
        if (!phaseDir) {
            throw new Error(`Phase ${phaseNumber} directory not found`);
        }
    ...
    async writeIssues(phaseNumber, issues) {
        const phaseDir = this.resolvePhaseDir(phaseNumber);
        if (!phaseDir) {
            throw new Error(`Phase ${phaseNumber} directory not found`);
        }
    ...
    async writeDiscovery(phaseNumber, params) {
        const phaseDir = this.resolvePhaseDir(phaseNumber);
        if (!phaseDir) {
            throw new Error(`Phase ${phaseNumber} directory not found`);
        }
...
//# sourceMappingURL=pre-planning-manager.js.map