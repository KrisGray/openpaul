...
function getApplyTaskContext(plan, metadata) {
...
function resolveActiveTaskIndex(metadata, totalTasks) {
    if (!metadata) {
        return null;
    }
    const rawIndex = getNumberValue(metadata.currentTaskIndex, metadata.activeTaskIndex, metadata.taskIndex, metadata.currentTaskNumber, metadata.activeTaskNumber);
    if (rawIndex === null) {
        return null;
    }
...
function resolveActiveTaskName(plan, metadata, activeTaskIndex) {
...
function resolveTimeInProgress(metadata, activeTaskIndex) {
...
function resolveTaskStartTime(metadata, activeTaskIndex) {
    if (!metadata) {
        return null;
    }
    const directTime = getNumberValue(metadata.currentTaskStartedAt, metadata.activeTaskStartedAt, metadata.taskStartedAt);
    if (directTime !== null) {
        return directTime;
    }
